...
| `PAN` | 🟡 | Pantheon page — visual pass (reopened) | 2026-07-13 |
| `VLT` | 🟡 | Vault → "Khaos Vortex" rename | 2026-07-13 |
| `MUS` | 🟡 | Museum chrome (no sidebar/chat) | 2026-07-13 |
| `KTX` | 🟡 | KhaoticText on all Vortex titles | 2026-07-13 |
| `CMP` | 🟢 | Component consolidation (shared Chip primitive) | 2026-07-13 |
...
`lg` 18 / `2xl` 24, all already in use somewhere in the app). That scale's
own step-to-step ratios sit in the 1.11–1.33 range.
...
Still open: waiting on review before wiring these five tokens into actual
component `text-*` classes app-wide — right now they exist only as a
reference in the Vault, nothing outside it consumes them yet, so no
...
surfaces). `copper`→**Eros** (primordial desire — the one warm, saturated
accent allowed to compete for attention, i.e. buttons).

`teal`→**Pontus** (the sea — deliberately kept cool and distant from Eros
on the hue wheel so it's never mistaken for a call to click). `sage`→
**Gaia** (the earth — growth, the quiet "done" state).
...
pass: dropped the circular backgrounds behind each deity's icon, enlarged
the icons (17px inside a 40px bordered circle → a bare 40px icon), moved
the icon into its own left column instead of sitting inline before the
name.

Since the complaint was about excess whitespace on the page generally,
not just the icons, also trimmed the shared `Chamber` wrapper's padding
...
The underlying rename is still **display-only**: the actual CSS variable
names in `index.css` (`--color-teal-*` etc.) and every Tailwind class
referencing them across an estimated 35+ files have not been touched
yet — that's a deliberately separate, larger, and riskier step being held
until the naming and visual treatment are both fully settled.
...
Replaced it with `/dev/vault`: an index page listing six numbered
"chambers" in Roman numerals (I–VI), each its own route (`/pantheon`,
`/chorus`, `/wellspring`, `/sigils`, `/forge`, `/threshold`), sharing a
small, deliberately minimal `Chamber`/`Section`/`Swatch` chrome kit
factored into `vaultUI.tsx` so each chamber page is mostly just its own
content.

No persistent sidebar nav inside the Vault — the index page's list of
chambers *is* the entire navigation system, the way a museum floor plan
works rather than a menu bar.
...
e.g. "khaos vortex · III"), and the sample text used in the Chorus page's
largest type-scale specimen.

Deliberately did **not** rename the underlying route paths (still
`/dev/vault/*`), the file names (`VaultIndexPage.tsx`, `vaultUI.tsx`),
the `MuseumFrame`/`Chamber` component names, or this backlog code itself
(still `VLT`) — those are all purely-internal identifiers with no
user-visible benefit to renaming. Full internal rename available on
request.
...
still visible alongside every Vault page, which directly fought the goal
of the Vault reading as its own space rather than a page bolted onto the
task manager.

Fixed at the routing level: moved every `/dev/vault*` route in `App.tsx`
...
mark fixed at the top-right as the only other interactive element on the
page.
...
(via `git mv`, preserving history), the default export name, and both of
its two existing real usages in the app (`KhaosLogo.tsx`'s sidebar
...
"Khaos Vortex" heading, each of the six chamber-row titles listed on that
same index page, and every individual chamber page's own large heading
(via the shared `Chamber` component, so all six chamber pages picked it
up from one change).

Verified in a real headless-browser render before pushing — confirmed the
per-character weight and width genuinely animate on these titles now, not
...
Right now, `EntityChip` (in the assistant/chat code), `ChangeBadge` (the
write-confirmation diff badges), and the various badge components
...
type scale (`TYP`) are both fully settled, since the primitive's styling
should be built on finished, stable tokens rather than values that might
still shift under it mid-build.
...
| ChaoticText / KhaoticText not visibly stretching | 2026-07-13 | 2026-07-13 |
...
which obscures the literal (lowercase) identifier.

Removed uppercase styling specifically from raw token/variable name
labels; left the general section-header uppercase convention alone since
that's a pre-existing, intentional app-wide style.

**Realistic sample content in gallery** — 2026-07-13 → 2026-07-13

The font specimens and "text as used today" reference were showing their
own Tailwind class names as the literal sample text.

Replaced with task-manager-shaped copy per font, then further replaced
with Khaos-mythology-themed sentences on request, all roughly matched in
...
saturation — a real risk for color-blind users and quick scanning. Moved
`rust`'s hue to 345° (`#b5483d`→`#b43c5a`), landing 42° from copper while
staying unambiguously red — arguably a better fit for its new name,
Tartarus, than the original.

**`--font-body` not loading** — 2026-07-13 → 2026-07-13

`--font-body` declared `'Inter'`, but no `@fontsource` package for it was
ever installed, so body text was silently falling back to `system-ui`
the entire time.
...
random rotation (Roboto Slab, Roboto Mono) turned out to have **no width
axis at all** in their actual font files — confirmed by reading each
package's `metadata.json` directly, not assumed.
...
expanded-black classes measured 100px vs. 271px wide in a real headless
render.
