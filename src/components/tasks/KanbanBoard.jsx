  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 border-b border-ink-700 px-3 py-2">
        ...
        <span className="text-xs font-semibold uppercase tracking-wide text-ink-400">{meta.label}</span>
  ...
  return (
    <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
      <div className="flex gap-3 overflow-x-auto pb-2">
        {STATUSES.map((status) => (
          <Column key={status} status={status} tasks={tasks.filter((t) => t.status === status)} onOpen={onOpenTask} />
        ))}
      </div>
    </DndContext>
  )
