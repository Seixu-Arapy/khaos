...
export function useAllTaskLogs() {
  return useQuery({ queryKey: ['taskLogs', 'all'], queryFn: () => timeTrackingApi.listAll() })
}
...
export function useTimerMutations() {
  ...
  return {
    start: useMutation({ mutationFn: (taskId) => timeTrackingApi.start(taskId), onSuccess: invalidateAll }),
    stop: useMutation({ mutationFn: timeTrackingApi.stop, onSuccess: invalidateAll }),
    updateLog: useMutation({
      mutationFn: ({ id, patch }) => timeTrackingApi.update(id, patch),
      onSuccess: invalidateAll,
    }),
    removeLog: useMutation({ mutationFn: timeTrackingApi.remove, onSuccess: invalidateAll }),
  }
}
