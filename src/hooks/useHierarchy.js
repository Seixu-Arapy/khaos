...
import { fieldsApi, projectsApi, sectionsApi, tasksApi, taskItemsApi, tasksSequenceApi, sectionsSequenceApi } from '../lib/api/hierarchy'
...
export function useTasksSequence() {
  return useQuery({ queryKey: ['tasksSequence'], queryFn: tasksSequenceApi.list })
}

export function useSectionsSequence() {
  return useQuery({ queryKey: ['sectionsSequence'], queryFn: sectionsSequenceApi.list })
}
...
export function useOrderedSectionIds(projectId, sections, sequenceEdges) {
  const ids = sections.filter((s) => s.project_id === projectId).map((s) => s.id)
  const edges = sequenceEdges
    .filter((e) => ids.includes(e.section_previous) && ids.includes(e.section_next))
...
export function useFieldMutations() {
  ...
  return {
    create: useMutation({ mutationFn: fieldsApi.create, onSuccess: invalidate }),
    update: useMutation({ mutationFn: ({ id, patch }) => fieldsApi.update(id, patch), onSuccess: invalidate }),
    remove: useMutation({ mutationFn: fieldsApi.remove, onSuccess: invalidate }),
  }
}
...
export function useProjectMutations() {
  ...
  return {
    ...
    update: useMutation({ mutationFn: ({ id, patch }) => projectsApi.update(id, patch), onSuccess: invalidate }),
    remove: useMutation({
...
export function useSectionMutations() {
  ...
  return {
    create: useMutation({ mutationFn: sectionsApi.create, onSuccess: invalidate }),
    update: useMutation({ mutationFn: ({ id, patch }) => sectionsApi.update(id, patch), onSuccess: invalidate }),
    remove: useMutation({
    ...
    reorder: useMutation({
      mutationFn: ({ projectId, orderedIds }) => sectionsApi.persistOrder(projectId, orderedIds),
...
export function useTaskMutations() {
  ...
  return {
    ...
    update: useMutation({ mutationFn: ({ id, patch }) => tasksApi.update(id, patch), onSuccess: invalidate }),
...
export function useTaskItemMutations(taskId) {
  const qc = useQueryClient()
  const invalidate = () => qc.invalidateQueries({ queryKey: ['taskItems', taskId] })
  return {
    create: useMutation({ mutationFn: taskItemsApi.create, onSuccess: invalidate }),
    update: useMutation({ mutationFn: ({ id, patch }) => taskItemsApi.update(id, patch), onSuccess: invalidate }),
    remove: useMutation({ mutationFn: taskItemsApi.remove, onSuccess: invalidate }),
  }
}
