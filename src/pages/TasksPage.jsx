...
export default function TasksPage() {
  ...
  const taggedTaskIds = useMemo(() => {
    ...
    return new Set(
      tagLinks.filter((l) => l.entity_type === 'task' && l.work_tag_id === Number(tagFilter)).map((l) => l.entity_id)
    )
  }, [tagLinks, tagFilter])
  ...
  const filtered = useMemo(() => {
    return tasks.filter((t) => {
      ...
      if (projectFilter !== 'all') {
        ...
        if (!section || section.project_id !== Number(projectFilter)) return false
      }
  ...
  }, [tasks, statusFilter, priorityFilter, projectFilter, taggedTaskIds, search, sectionsById])
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-display text-2xl text-ink-100">All tasks</h1>
          ...
          {VIEWS.map((v) => (
            <button
              ...
              className={clsx(
                ...
                view === v.id ? 'bg-ink-700 text-ink-100' : 'text-ink-500 hover:text-ink-200'
              )}
            >
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <span className="ml-auto text-xs text-ink-600">{filtered.length} tasks</span>
      </div>

      {view === 'list' && (
        <TaskList tasks={filtered} projectsById={projectsById} sectionsById={sectionsById} onOpenTask={openTask_} />
      )}
      {view === 'kanban' && <KanbanBoard tasks={filtered} onOpenTask={openTask_} />}
      {view === 'priority' && <PriorityBoard tasks={filtered} onOpenTask={openTask_} />}
