...
export async function getOpenApiSpec({ forceRefresh = false } = {}) {
  ...
  if (!res.ok) throw new Error(`Could not load schema spec from Supabase (status ${res.status})`)
...
export async function searchSchema(query) {
  ...
  const matchedTables = Object.entries(defs)
    ...
    .map(([tableName, def]) => ({
      ...
      columns: Object.entries(def.properties || {}).map(([col, prop]) => describeProperty(col, prop)),
    }))
