...
export function parseQuickAdd(rawInput, projects = []) {
  ...
  let m = text.match(new RegExp(`\\bin (\\d+) (day|days|week|weeks)\\b(?:\\s+at\\s+(${timePattern}))?`, 'i'))
  if (m) {
  ...
  // --- "next <weekday>" ---
  if (!dueDate) {
    m = text.match(new RegExp(`\\bnext (${DAY_NAMES.join('|')})\\b(?:\\s+at\\s+(${timePattern}))?`, 'i'))
    if (m) {
  ...
  if (!dueDate) {
    m = text.match(new RegExp(`\\b(${DAY_NAMES.join('|')})\\b(?:\\s+at\\s+(${timePattern}))?`, 'i'))
    if (m) {
      const dayIdx = DAY_NAMES.indexOf(m[1].toLowerCase())
      const base = nextWeekday(new Date(), dayIdx)
      dueDate = applyTimeOfDay(base, m[2])
  ...
  // --- "today" / "tomorrow" ---
  if (!dueDate) {
    m = text.match(new RegExp(`\\b(today|tomorrow)\\b(?:\\s+at\\s+(${timePattern}))?`, 'i'))
    if (m) {
      const d = new Date()
